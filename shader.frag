//============================================================
// STUDENT NAME: Clarissa Lee Zi Xin
// NUS User ID.: A0205284J
// COMMENTS TO GRADER:
//
// ============================================================
...
void drawBrickCube()
{
    if (gl_FrontFacing) {
        ...
        vec3 T; 
        vec3 B;
        compute_tangent_vectors(necNormal, ecPosition, vec2(v2fTexCoord), T, B);

        // Perturbation vector in tangent space.
        vec3 tanPerturbedNormal = vec3(texture(BrickNormalMap, vec2(v2fTexCoord)));
        tanPerturbedNormal = tanPerturbedNormal * 2 - 1;

        // Transform perturbation vector to eye space.
        vec3 ecPerturbedNormal = normalize(tanPerturbedNormal.x * T + 
                                           tanPerturbedNormal.y * B + 
                                           tanPerturbedNormal.z * necNormal * DeltaNormal_Z_Scale);

        // Use ecPerturbedNormal as normal vector in lighting computation using Phong Reflection Model.
        ...
        vec4 ambientDiffuseColor = texture(BrickDiffuseMap, vec2(v2fTexCoord));
        FragColor = LightAmbient * ambientDiffuseColor +
                    LightDiffuse * ambientDiffuseColor * N_dot_L +
                    LightSpecular * vec4(BrickSpecularMaterial, 1.0) * spec;
    }
    else discard;
}
...
void drawWoodenCube()
{
    if (gl_FrontFacing) {
        ...
        if (length(p) >= MirrorRadius) {
            ...
            vec4 ambientDiffuseColor = texture(WoodDiffuseMap, vec2(v2fTexCoord));
            FragColor = LightAmbient * ambientDiffuseColor +
                        LightDiffuse * ambientDiffuseColor * N_dot_L +
                        LightSpecular * vec4(WoodSpecularMaterial, 1.0) * spec;
        } else {
            ...
            // Transform reflection vector to World Space.
            vec4 wcReflectVec = inverse(ViewMatrix) * vec4(ecReflectVec, 0.0);

            // Use world-space reflection vector to access environment cubemap.
            FragColor = texture(EnvMap, vec3(wcReflectVec));
        }
